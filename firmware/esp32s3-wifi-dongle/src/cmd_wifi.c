...
extern volatile uint32_t led_traffic_rx, led_traffic_tx;
...
/* Retry connecting with the NVS-persisted credentials until it works.
 * `reconnect` is cleared by `sta -d`, which stops the retries. */
static void wifi_auto_connect_task(void *arg)
{
    wifi_config_t cfg = {0};
    vTaskDelay(pdMS_TO_TICKS(3000));
    while (true) {
        if (s_wifi_is_connected || !reconnect) {
            vTaskDelay(pdMS_TO_TICKS(10000));
            continue;
        }
        if (esp_wifi_get_config(WIFI_IF_STA, &cfg) == ESP_OK && cfg.sta.ssid[0] != '\0') {
            ESP_LOGI(TAG, "auto-connect: trying saved SSID %s", (char *)cfg.sta.ssid);
            esp_wifi_connect();
        }
        vTaskDelay(pdMS_TO_TICKS(10000));
    }
